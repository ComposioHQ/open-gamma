...
export default function Chat() {
  const [selectedModel, setSelectedModel] = useLocalStorage<string>(STORAGE_KEYS.model, DEFAULT_MODEL);
  const [sidebarCollapsed, setSidebarCollapsed] = useLocalStorage<boolean>('open-gamma:sidebar:collapsed', false);
  ...
  const convertToTypedMessages = useCallback((msgs: typeof messages): TypedUIMessage[] => {
    return msgs.map((m) => ({
      id: m.id,
  ...
  const debouncedSave = useCallback((chatId: string, msgs: typeof messages) => {
    if (saveTimeoutRef.current) {
      clearTimeout(saveTimeoutRef.current);
    }
    saveTimeoutRef.current = setTimeout(() => {
      if (msgs.length > 0) {
        saveMessages(chatId, convertToTypedMessages(msgs));
      }
    }, 1000);
  }, [saveMessages, convertToTypedMessages]);
  ...
  useEffect(() => {
    ...
    if (activeChatId && messages.length > 0 && !isLoading) {
      debouncedSave(activeChatId, messages);
    }
  }, [messages, activeChatId, isLoading, debouncedSave]);
  ...
  return (
    <div className="h-screen w-full flex bg-ui-card font-sans overflow-hidden">
      ...
      {/* Main Chat Area */}
      <div className={`flex-1 flex flex-col bg-ui-card relative z-10 transition-all duration-300 ${
